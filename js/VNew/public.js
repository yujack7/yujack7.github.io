...
function add_reduce_num3(arithmetic){
	...
	$('#pro_total').html('<b>总计</b><span class="newPrice"><em>'+ currencyType + allMoney +'</em>'+prounit+'</span>');
}
...
function keyboard_num(){
	...
	if(!is_number_var.test(reality_num) || !is_number_zheng.test(reality_num) || reality_num > tmp_num || reality_num<tmp_num2){
		...
		$('#pro_total').html('<b>总计</b><span class="newPrice"><em>'+ currencyType + shop_price + '</em></span>');
	}else{
		$('#pro_num').val(reality_num);
		$('#pro_total').html('<b>总计</b><span class="newPrice"><em>'+ currencyType +  nCount.mul(Number(shop_price), Number(reality_num)) + '</em></span>');
	  }
}
        ...
        function (data) {
			...
			if (num == 0) {
				...
				if(Minimum_ord > num){
					...
					$("#pro_total").html('<b>总计</b><span class="newPrice"><em>'+ currencyType + total_price + '</em></span>');
					//获取选中推荐产品的价格
